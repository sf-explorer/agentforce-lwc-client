...
export default class AgentforceChat extends LightningElement {
  ...
  @api markdownRenderer = "custom";
  @api voiceInputEnabled = false;
  @api voiceLanguage = "en-US";
  ...
  get normalizedMessages() {
    return this.messages.map((message, index) => ({
      ...
      hasCitations:
        this.showCitations && message.citations && message.citations.length > 0
    }));
  }

  get useCustomMarkdownRenderer() {
    return this.markdownEnabled && this.markdownRenderer === "custom";
  }

  get containerStyle() {
    return `--agentforce-chat-min-height:${this.normalizeHeightValue(
      this.minHeight
    )};--agentforce-chat-max-height:${this.normalizeHeightValue(this.maxHeight)};`;
  }
      ...
      if (event.key === "Enter" && !event.shiftKey && this.isCommandMode) {
        const exactMatch = this.commandSuggestions.find(
          (item) =>
            ...
            this.draftMessage.trim().toLowerCase()
        );
        if (!exactMatch) {
          event.preventDefault();
          this.applyActiveCommandSuggestion();
  ...
  handleVoiceError(event) {
    this.dispatchEvent(
      new CustomEvent("voiceerror", {
        detail: {
          message: event.detail?.message || "Voice capture failed.",
  ...
  async handleSend(text, options = {}) {
    ...
    const messageWithContext = this.withAgentContext(
      skillResolution.resolvedText
    );
    ...
    try {
      ...
      const errorMessage = this.appendMessage(
        "assistant",
        parsedError.message,
        {
  ...
  resolveSkillMentions(text) {
    ...
    const labels = Object.keys(skillsByLabel).sort(
  ...
  parseError(error) {
    const rawMessage =
      error?.body?.message || error?.message || "UNKNOWN_ERROR";
    ...
    rendered = rendered.replace(
      ...
      (_match, label, href) => {
        if (href.startsWith("/")) {
          ...
          const isRecordPath =
            /^\/lightning\/r\/(?:[^/]+\/)?[a-zA-Z0-9]{15,18}\/(view|edit|clone)$/i.test(
              href
            );
          if (isRecordPath) {
            return `<a href="${absoluteHref}">${label}</a>`;
          }
